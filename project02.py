...
# Button layout (fixed "=" and "+" positions)
buttons = [
...
clear_btn.grid(row=5, column=0, columnspan=4, sticky="nsew", pady=10)
...
for (text, row, col) in buttons:
    if text == "=":
        btn = tk.Button(root, text=text, font=("Arial", 18),
                        height=2, width=5, command=equalpress,
                        bg="lightgreen")
    else:
        btn = tk.Button(root, text=text, font=("Arial", 18),
                        height=2, width=5,
    ...
    # grid must ALWAYS be outside the if/else
    btn.grid(row=row, column=col, padx=5, pady=5)

# Clear button
clear_btn = tk.Button(root, text="C", font=("Arial", 18),
                      height=2, width=22, bg="lightcoral",
                      command=clear)
